...
sp_time	E_get_time(void)
{
        ...
        ret = gettimeofday((struct timeval *)((char*)&Now), (void *)&dummy_tz );        if ( ret < 0 ) Alarm( EXIT, "E_get_time: gettimeofday problems.\n" );
...
static	const sp_time		mili_sec 	= {     0, 1000};
	...
	time_event		*temp_ptr;
        int                     first=1;
	int                     count, handled_non_low, num_set_start;
#ifdef TESTTIME
	...
	if(POLL_NON_LOW_PRIORITY_THRESHOLD == 0) {
	  handled_non_low = 1;
	}
#if 1
	while(!handled_non_low) {
	  /* JK ADDED: Poll after handling events once */
	  for(i = 0; i < NUM_FDTYPES; i++)
	    current_mask[i] = Fd_mask[i];
	  ...
	  wait_timeout.tv_usec = zero_sec.usec;
	  num_set = select(FD_SETSIZE, &current_mask[READ_FD], &current_mask[WRITE_FD], 
			   &current_mask[EXCEPT_FD],  &wait_timeout);

	  num_set_start = num_set;
	  ...
	  /* Handle all high and medium priority fd events */
	  for(i = NUM_PRIORITY-1; i > LOW_PRIORITY && num_set > 0; i--) {
	    for(j = 0; j < Fd_queue[i].num_fds && num_set > 0; j++) {
	      fd      = Fd_queue[i].events[j].fd;
	      fd_type = Fd_queue[i].events[j].fd_type;
	      if(FD_ISSET(fd, &current_mask[fd_type])) {
		...
		if(count == POLL_NON_LOW_PRIORITY_THRESHOLD) {
		  Alarm(PRINT, "Reached threshold of %d\n", count);
		  handled_non_low = 1;
		  goto end_for;
		  //break;
		}
	...
	end_for:
	  /* If we finished the non-low events and there were none, then try to handle low events */
	  if(num_set == num_set_start) {
	    handled_non_low = 1;
	    break; //Out of while
	  }
	}
	...
	while(1) {
	  ...
	  wait_timeout.tv_sec  = zero_sec.sec;
	  wait_timeout.tv_usec = POLL_USEC;
	  num_set = select(FD_SETSIZE, &current_mask[READ_FD], &current_mask[WRITE_FD], 
			   &current_mask[EXCEPT_FD],  &wait_timeout);
	  ...
	  for(j = 0; j < Fd_queue[LOW_PRIORITY].num_fds && num_set > 0; j++) {
	    ...
	    if(FD_ISSET(fd, &current_mask[fd_type])) {
	      ...
	      if(count == POLL_LOW_PRIORITY_THRESHOLD) {
		Alarm(PRINT, "Reached threshold of %d\n", count);
		goto end_while;
		//break;
	      }
